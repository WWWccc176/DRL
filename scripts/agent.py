...
def parse_challenge_file(filepath):
    ...
    if total >= 2 and int(math.isqrt(total - 2))**2 == (total - 2):
        ...
        if dim * dim != total:
            raise ValueError(f"parse_challenge_file: cannot infer square matrix, total={total}")
        matrix_data = numbers

    if len(matrix_data) != dim * dim:
        raise ValueError(f"parse_challenge_file: size mismatch, dim={dim}, got={len(matrix_data)}")
...
class DQNAgent:
    def __init__(self, state_dim, action_dim, use_amp=True, batch_size=256, updates_per_step=2):
        self.device = device
...
# ------------------------------
class LatticeEnv:
    def __init__(self, matrix_path, max_steps=20, metrics_every=5, debug_step=False,
                 alpha=1.0, beta_w=0.3, gamma=0.1, cost_w=0.002):
...
def train(env, agent, episodes=200, print_every=10, debug_each_step=False):
    ...
    for ep in range(1, episodes + 1):
        ...
        while not done:
            ...
            if debug_each_step:
                print(f"    step={info['step']:02d} beta={info['beta']} pos={info['pos']} r={r:.4f} ghR={info['gh_ratio_robust']:.6f}")
...
# Run
# ------------------------------
env = LatticeEnv(TRAIN_FILE, max_steps=25, metrics_every=5, debug_step=False,
                 alpha=1.0, beta_w=0.3, gamma=0.1, cost_w=0.002)

print(f"🤖 Action space size = {env.num_actions} (example first 10): {env.action_list[:10]}")
