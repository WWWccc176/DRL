...
# ------------------------------
# The server has much more CPU/GPU capacity than the original laptop.
# Keep each fplll/Rust worker single-threaded, then use many workers for CPU parallelism.
# PyTorch gets its own thread setting below via torch.set_num_threads(...).
NATIVE_WORKER_THREADS = int(os.environ.get("A8_NATIVE_WORKER_THREADS", "1"))
MAIN_TORCH_THREADS = int(os.environ.get("A8_MAIN_TORCH_THREADS", "16"))
os.environ["OMP_NUM_THREADS"] = str(NATIVE_WORKER_THREADS)
os.environ["MKL_NUM_THREADS"] = str(NATIVE_WORKER_THREADS)
os.environ["OPENBLAS_NUM_THREADS"] = str(NATIVE_WORKER_THREADS)
os.environ["RAYON_NUM_THREADS"] = str(NATIVE_WORKER_THREADS)
...
DEFAULT_ENVS_PER_SEED = int(os.environ.get("A8_ENVS_PER_SEED", "6"))
DEFAULT_BATCH_SIZE = int(os.environ.get("A8_BATCH_SIZE", "1024"))
DEFAULT_REPLAY_CAPACITY = int(os.environ.get("A8_REPLAY_CAPACITY", "200000"))
...
def reset_model_noise(model):
    unwrap_model(model).reset_noise()


def clean_state_dict(state_dict):
    # Compatible with both plain nn.Module and nn.DataParallel checkpoints.
...
def find_project_root():
    """Find DRL project root robustly even if this script is under scripts/agent/."""
    here = Path(__file__).resolve()
    for base in [here.parent] + list(here.parents):
        if (base / "dataset").is_dir() and (
            (base / "rustcore").exists() or (base / "results").exists()
        ):
            return str(base)
    # Fallback to the original a8 convention: DRL/scripts/a8.py -> DRL
...
class AxialCNN_DuelingDDQN(nn.Module):
    def __init__(self, max_dim, action_dim):
        ...
        # Larger server profile: deeper and wider CNN/MLP while keeping the same state/action logic.
        self.col_conv = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=(5, 1), padding=(2, 0)),
            nn.LeakyReLU(0.01),
            nn.Conv2d(
                64,
                128,
        ...
        self.se = SE_Block(128, reduction=4)
        ...
        cnn_flat_size = 128 * self.grid_size * self.grid_size

        self.fusion = nn.Sequential(
            nn.Linear(cnn_flat_size + 128, 768),
            nn.LeakyReLU(0.01),
            nn.Linear(768, 512),
            nn.LeakyReLU(0.01),
        )
...
class DQNAgent:
    def __init__(
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 1:
            ...
            self.q_net = nn.DataParallel(self.q_net)
    ...
    def save_checkpoint(self, model_path):
        torch.save(model_state_dict(self.q_net), model_path)
    ...
    def load_checkpoint(self, model_path):
        ...
        try:
            load_model_state(self.q_net, state)
            load_model_state(self.target_net, state)
        except RuntimeError as e:
            print(f"[Warning] Skip incompatible checkpoint: {model_path}")
            print(f"          {str(e).splitlines()[0]}")

    def save_training_state(self, path, extra=None):
        """保存可续训状态；不保存 replay buffer，避免 pth 过大"""
        payload = {
            "q_net": model_state_dict(self.q_net),
            "target_net": model_state_dict(self.target_net),
    ...
    def load_training_state(self, path):
        ...
        try:
            load_model_state(self.q_net, payload["q_net"])
            load_model_state(self.target_net, payload["target_net"])
            self.optimizer.load_state_dict(payload["optimizer"])
            self.scheduler.load_state_dict(payload["scheduler"])
        except RuntimeError as e:
            print(f"[Warning] Skip incompatible resume checkpoint: {path}")
            print(f"          {str(e).splitlines()[0]}")
            return {}
    ...
    def replay(self):
        ...
        with torch.no_grad():
            ...
            reset_model_noise(self.target_net)
        ...
        with torch.no_grad():
            for tp, p in zip(
                unwrap_model(self.target_net).parameters(),
                unwrap_model(self.q_net).parameters(),
            ):
...
class LatticeEnv:
    ...
    def reset(self):
        # 每次 reset 从本 env 绑定的 seed 列表中取种子；a7up 下每个 env 只绑定一个 seed
...
class SubprocVecEnv:
    def __init__(self, num_envs, matrix_paths, max_dim=250, envs_per_seed=2):
        """
        matrix_paths: list[str] — 所有种子文件路径。
        num_envs: int — 环境数。a7up 固定为每个 seed 分配 envs_per_seed 个 env。
        envs_per_seed: int — 每个 seed 专属 env 数量，默认 2。
        """
        if isinstance(matrix_paths, str):
            matrix_paths = [matrix_paths]

        expected_envs = len(matrix_paths) * envs_per_seed
        if num_envs != expected_envs:
            raise ValueError(
                f"num_envs must equal len(matrix_paths) * envs_per_seed. "
                f"Got num_envs={num_envs}, seeds={len(matrix_paths)}, "
                f"envs_per_seed={envs_per_seed}, expected={expected_envs}."
            )

        self.num_envs = num_envs
        self.envs_per_seed = envs_per_seed

        # 每两个 env 固定绑定同一个 seed:
        # env 0,1 -> seed 0; env 2,3 -> seed 1; ...
        self.env_seed_indices = [eid // envs_per_seed for eid in range(num_envs)]
        self.env_matrix_paths = [[matrix_paths[sidx]] for sidx in self.env_seed_indices]
        self.env_assignments = [
            {
                "env_id": eid,
                "seed_index": sidx,
                "seed_file": os.path.basename(matrix_paths[sidx]),
            }
            for eid, sidx in enumerate(self.env_seed_indices)
        ]
        ...
        self.processes = [
            mp.Process(
                target=env_worker,
                args=(work_remote, remote, env_paths, max_dim),
            )
            for work_remote, remote, env_paths in zip(
                self.work_remotes, self.remotes, self.env_matrix_paths
            )
        ]
    ...
    for ep in range(start_ep, episodes + 1):
        ...
        if ep % print_every == 0:
            ...
            if best_ep_eid is not None:
                ...
                assignment = getattr(vec_env, "env_assignments", None)
                seed_desc = ""
                if assignment is not None:
                    seed_desc = f" | seed_file: {assignment[best_ep_eid]['seed_file']}"
                print(
                    f"    Env{best_ep_eid}{seed_desc} | "
                    f"best ratio in episode: {best_ep_ratio:.8f}"
                )
...
def run_experiment(
    ...
    dataset_dir,
    results_dir,
    envs_per_seed=DEFAULT_ENVS_PER_SEED,
    batch_size=DEFAULT_BATCH_SIZE,
):
    max_dim = dim
    ...
    all_files = sorted(glob.glob(pattern), key=seed_sort_key)
    if not all_files:
        print(f"[Dim {dim}] No files found matching {pattern}")
        return

    num_envs = len(all_files) * envs_per_seed

    print(
        f"[Dim {dim}] {len(all_files)} seeds, {num_envs} envs "
        f"({envs_per_seed} envs per seed), max_dim={max_dim}, "
        f"batch_size={batch_size}"
    )

    version_name = "a8_server"
    ...
    # More envs per seed for the dual-CPU server; seed binding logic is unchanged.
    ...
    for item in vec_env.env_assignments:
        print(
            f"  Env{item['env_id']:02d} -> "
            f"seed_index={item['seed_index']} | {item['seed_file']}"
        )

    # temp_env 只用于读取 action_dim / max_steps；用一个 seed 即可，避免主进程预加载全部 seed 占内存
    temp_env = LatticeEnv([all_files[0]], max_dim=max_dim)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dim-min", type=int, default=50, help="Minimum SVP dimension to run."
    )
    parser.add_argument(
        "--dim-max", type=int, default=70, help="Maximum SVP dimension to run."
    )
    parser.add_argument(
        "--envs-per-seed",
        type=int,
        default=DEFAULT_ENVS_PER_SEED,
        help="Number of dedicated env processes bound to each seed file.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=DEFAULT_BATCH_SIZE,
        help="DQN replay batch size. 1024 is intended for the 4-GPU server.",
    )
    args = parser.parse_args()

    if args.dim_min > args.dim_max:
        raise ValueError(
            f"dim-min must be <= dim-max, got {args.dim_min} > {args.dim_max}"
        )
    if args.envs_per_seed <= 0:
        raise ValueError("envs-per-seed must be positive")
    ...
    print(
        f"[A8 server profile] dim={args.dim_min}..{args.dim_max}, "
        f"envs_per_seed={args.envs_per_seed}, batch_size={args.batch_size}, "
        f"main_torch_threads={MAIN_TORCH_THREADS}"
    )
    ...
    for dim in range(args.dim_min, args.dim_max + 1):
        run_experiment(
            ...
            DATASET_DIR,
            RESULTS_DIR,
            envs_per_seed=args.envs_per_seed,
