...
def train_all(
    ...
    highest_seed = min(seed for dim, seed in vec_env.dataset_pairs if dim == highest_dim)
    ...
    def print_cycle_detail(cycle_no: int):
        ...
        for dim in dataset_dims:
            candidates = [
                info for info in global_info.values() if info["dim"] == dim
            ]
            if candidates:
    ...
    initial_actions = agent.act_envs(state_by_eid, eps_now())
    for env_id in range(num_envs):
        prev_s[env_id], prev_a[env_id] = states[env_id], initial_actions[env_id]
        vec_env.send_one(env_id, initial_actions[env_id])
    ...
    while updates < total_updates:
        ...
        actions = agent.act_envs(newly, eps_now())
        for env_id in newly:
            prev_s[env_id], prev_a[env_id] = states[env_id], actions[env_id]
            vec_env.send_one(env_id, actions[env_id])
            pending.add(env_id)

        if env_steps % log_every < len(ready):
            best_min = min(global_best.values()) if global_best else float("inf")
            reached = sum(1 for value in global_best.values() if value < goal_threshold)
            history["best_min"].append(best_min)
            rate = env_steps / max(1e-6, time.time() - t_start)
            status(
                f"upd {updates}/{total_updates} | cycles {cycles_completed} | "
                f"ε{eps_now():.3f} | loss{latest_loss:.4f} | "
        ...
        if env_steps % save_every < len(ready):
            agent.save(
                os.path.join(results_dir, f"{AGENT_VERSION}.pth"),
                extra={
                    "updates": updates,
                    "env_steps": env_steps,
    ...
    infos = list(global_info.values())
    for dim in dataset_dims:
        save_dimension_summary(results_dir, dim, infos, goal_threshold)
    save_final_summary(results_dir, infos, goal_threshold)
    plot_training_history(results_dir, history, goal_threshold)
