...
def train(
    ...
    for ep in range(start_ep, episodes + 1):
        ...
        # ---- 打印 ----
        if ep % print_every == 0:
            sys.stdout.write("\r\033[K")  # 先清掉状态行
            print(f"[{'=' * 15} Ep {ep} (Dim {dim}) {'=' * 15}]")
            print("  Seed Progress:")
            ...
            # 仍保留日志到 checkpoint，但不再整屏打印 History 流
            current_log = (
        ...
        _status(
            f"Dim{dim} Ep {ep}/{episodes} | ε:{epsilon:.3f} | "
            f"R:{avg_ep_reward:7.2f} | Loss:{history['loss'][-1]:.4f} | "
            f"Best:{best_global_ratio:.6f} | "
            f"Seeds:{reached_count}/{goal_count_needed}"
        )
...
# run_experiment
# ------------------------------
def run_experiment(dim, dataset_dir, results_dir, num_envs=18):
    max_dim = dim
    ...
    print(
        ...
        f"(2 envs per seed), max_dim={max_dim}"
    )
    ...
    # ★ a7up：每两个 env 专门对应一个 seed
    vec_env = SubprocVecEnv(num_envs, all_files, max_dim=max_dim, envs_per_seed=2)
