...
class LatticeEnv:
    ...
    def step(self, action_idx):
        ...
        if gain <= 0.001 and breakthrough_bonus == 0.0:
            penalty = -1.0
            if getattr(self, "last_pos", None) == pos:
                penalty -= 4.0
            reward += penalty
        self.last_pos = pos
...
def train(
    ...
    for ep in range(1, episodes + 1):
        ...
        if global_best_ratio < best_known_ratio:
            ...
            print(
                f"agent5HisBest find {global_best_ratio:.4f}! weight, memory and txt backed up"
            )
...
def run_experiment(dim, dataset_dir, results_dir, num_envs=4):
    ...
    best_file_path = os.path.join(
        results_dir, f"A5_2UP_best_results_low_dim{dim}.txt"
    )  # 提前定义路径
