...
# 严格按照原代码的顺序定义阶段
stages = [("LLL", 0), ("BKZ", 10), ("BKZ", 15), ("BKZ", 20), ("BKZ", 25), ("BKZ", 30), ("BKZ", 35)]
...
for dim in dims:
    ...
    for method, param in stages:
        ...
        res = my_cpp_module.run_reduction(current_str, method, param)
...
fig, axes = plt.subplots(nrows=len(valid_dims), ncols=len(stage_names), figsize=(28, 5 * len(valid_dims)))
# 处理单行情况
if len(valid_dims) == 1: axes = np.expand_dims(axes, axis=0)
...
for i, dim in enumerate(valid_dims):
    for j, stage_name in enumerate(stage_names):
        ...
        if key in plot_data:
            ...
            # 绘图
            im = ax.imshow(plot_mat, cmap=cmap, vmin=0, vmax=0.7, interpolation='nearest')
            ...
            ax.set_xlabel(info_text, fontsize=11, fontweight='medium', labelpad=8, linespacing=1.4)
        else:
            ax.axis('off')
