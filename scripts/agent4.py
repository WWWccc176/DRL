...
class LatticeEnv:
    ...
    def _get_state_and_update_best(self, mat_str, rust_info):
        ...
        for i in range(self.dim):
            tokens[i, 0] = gs_feat[i]  # 第一个特征是 GS 范数
            # 填入与之前向量的余弦夹角 (下三角部分)
            tokens[i, 1 : i + 1] = C[i, :i]
...
def train(
    ...
    for ep in range(1, episodes + 1):
        ...
        if global_best_ratio < best_known_ratio:
            ...
            model_path = os.path.join(save_dir, f"best_model_dim{dim}.pth")
            memory_path = os.path.join(save_dir, f"best_memory_dim{dim}.pkl")

            # 破纪录时，把模型和包含突破路径的优质经验池全存下来
            agent.save_checkpoint(model_path, memory_path)
            print(
                f"✨ HisBest find {global_best_ratio:.4f}! weight and memory backed up"
            )
...
def run_experiment(dim, dataset_dir, results_dir, num_envs=4):
    ...
    model_path = os.path.join(results_dir, f"best_model_dim{dim}.pth")
    memory_path = os.path.join(results_dir, f"best_memory_dim{dim}.pkl")
...
if __name__ == "__main__":
    ...
    for dim in DIMS_TO_RUN:
        run_experiment(dim, DATASET_DIR, RESULTS_DIR, num_envs=4)
