...
struct hw {
    static constexpr int ssd_num = 1;
    static constexpr const char *ssd_name_list[ssd_num] = {"0"};
    static inline const char *ssd_name(int) { return "0"; }

    static constexpr int gpu_num = LATTICE_GPU_NUM;
    static_assert(gpu_num >= 1 && gpu_num <= 8, "LATTICE_GPU_NUM must be in [1,8]");
    static constexpr int gpu_id_list[8] = {0,1,2,3,4,5,6,7};
    static inline int gpu_ptr(int tid, int num_threads) {
        int p = (tid * gpu_num) / (num_threads > 0 ? num_threads : 1);
        return p >= gpu_num ? gpu_num - 1 : p;
    }
    static inline int gpu_ptrl(int tid, int sid, int num_threads, int threads_per_buc) {
        if (sid == -1) return gpu_ptr(tid, num_threads);
        return (tid * threads_per_buc + sid) % gpu_num;
    }
...
#define POOL_HD_LOG_LEVEL ll_warn
#define REDUCER_LOG_LEVEL ll_warn
#define BUCKETER_LOG_LEVEL ll_warn
#define PWC_LOG_LEVEL ll_warn
#define BWC_LOG_LEVEL ll_warn
#define SWC_LOG_LEVEL ll_warn
#define DHB_LOG_LEVEL ll_warn
#define DHR_LOG_LEVEL ll_warn
...
#define PWC_SSD_SLIMIT (2ULL << 40)
#define PWC_DRAM_SLIMIT ((uint64_t)LATTICE_PWC_DRAM_GB << 30)
...
#define BWC_SSD_SLIMIT (2ULL << 40)
#define BWC_DRAM_SLIMIT ((uint64_t)LATTICE_BWC_DRAM_GB << 30)
...
#define SWC_SSD_SLIMIT (2ULL << 40)
#define SWC_DRAM_SLIMIT ((uint64_t)LATTICE_SWC_DRAM_GB << 30)
#define SWC_DEFAULT_MAX_CACHED_CHUNKS (SWC_DRAM_SLIMIT / 8192ULL / 190ULL)
#define SWC_MAX_PARALLEL_SYNC_CHUNKS 5

#define RED_MIN_CSD16 128
#define RED_MAX_NUM_THREADS 32
#define RED_GRAM_SLIMIT (12ULL << 30)
#define BGJ1_RED_DEFAULT_NUM_THREADS 24
#define BGJ2_RED_DEFAULT_NUM_THREADS 24
#define BGJ3_RED_DEFAULT_NUM_THREADS 12
...
#define BGJ_L0_MAX_BATCH0 2048
#define BGJ_L0_MIN_BATCH0 16
...
#define UT_TABLE_DRAM_SLIMIT ((uint64_t)LATTICE_UT_TABLE_DRAM_GB << 30)
#define UT_BUFFER_DRAM_SLIMIT ((uint64_t)LATTICE_UT_BUFFER_DRAM_GB << 30)
...
#define DHB_DEFAULT_NUM_THREADS 16
#define DHR_DEFAULT_NUM_THREADS 24
#define DHB_GRAM_SLIMIT (1ULL << 30)
#define DHR_GRAM_SLIMIT (12ULL << 30)
